...
options.priors.muTheta = [0;0];
options.priors.SigmaPhi = 10*eye(dim.n_phi);
options.priors.SigmaPhi(3,3) = 3;
...
noise=0;
hs.val.SS = 1/3; %(eval(tmat{1}{1})+eval(tmat{2}{1}))/2;
options.priors.muX0(hs.map.SS) = hs.val.SS;
for i = 1:3
    hs.val.SAS{i} = 1/3; %(eval(tmat{3}{i})+eval(tmat{4}{i}))/2;
